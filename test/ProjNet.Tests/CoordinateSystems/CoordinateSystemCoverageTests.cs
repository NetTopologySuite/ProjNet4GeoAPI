...
public class CoordinateSystemCoverageTests
{
    ...
    /// <summary>
    /// Verifies that Name setter updates the value.
    /// </summary>
    [Fact]
    public void Info_NameSetter_UpdatesValue()
    {
        PrimeMeridian pm = Factory.CreatePrimeMeridian("Original", AngularUnit.Degrees, 0.0);
        pm.Name = "Updated";

        Assert.Equal("Updated", pm.Name);
    }

    /// <summary>
    /// Verifies that Authority setter updates the value.
    /// </summary>
    [Fact]
    public void Info_AuthoritySetter_UpdatesValue()
    {
        var unit = new LinearUnit(1.0, "test", string.Empty, -1, string.Empty, string.Empty, string.Empty);
        unit.Authority = "CUSTOM";

        Assert.Equal("CUSTOM", unit.Authority);
    }

    /// <summary>
    /// Verifies that AuthorityCode setter updates the value.
    /// </summary>
    [Fact]
    public void Info_AuthorityCodeSetter_UpdatesValue()
    {
        var unit = new LinearUnit(1.0, "test", string.Empty, -1, string.Empty, string.Empty, string.Empty);
        unit.AuthorityCode = 42;

        Assert.Equal(42, unit.AuthorityCode);
    }

    /// <summary>
    /// Verifies that Alias property can be set and retrieved.
    /// </summary>
    [Fact]
    public void Info_AliasSetter_UpdatesValue()
    {
        var unit = new LinearUnit(1.0, "test", string.Empty, -1, string.Empty, string.Empty, string.Empty);
        unit.Alias = "myAlias";

        Assert.Equal("myAlias", unit.Alias);
    }

    /// <summary>
    /// Verifies that Abbreviation property can be set and retrieved.
    /// </summary>
    [Fact]
    public void Info_AbbreviationSetter_UpdatesValue()
    {
        var unit = new LinearUnit(1.0, "test", string.Empty, -1, string.Empty, string.Empty, string.Empty);
        unit.Abbreviation = "tst";

        Assert.Equal("tst", unit.Abbreviation);
    }

    /// <summary>
    /// Verifies that Remarks property can be set and retrieved.
    /// </summary>
    [Fact]
    public void Info_RemarksSetter_UpdatesValue()
    {
        var unit = new LinearUnit(1.0, "test", string.Empty, -1, string.Empty, string.Empty, string.Empty);
        unit.Remarks = "Some remark";

        Assert.Equal("Some remark", unit.Remarks);
    }
