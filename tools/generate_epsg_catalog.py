...
def extract_operation_data(zip_path: Path):
    ...
    with zipfile.ZipFile(zip_path, 'r') as zf:
        for info in sorted(zf.infolist(), key=lambda i: i.filename):
            ...
            operations.append((operation_type, operation_code, int(src_ids[-1]), int(tgt_ids[-1]), accuracy, method_name, parameter_file_name))

    operations.sort(key=lambda r: (r[0], r[1]))
    return operations
...
def emit(output_path: Path, zip_name: str, catalog, operations):
    lines = []
    ...
    struct_defs = {
        ...
        'EpsgOperationRecord': 'EpsgOperationType operationType, int operationCode, int sourceSrid, int targetSrid, double accuracy, string methodName, string parameterFileName',
    }
    ...
    lines.append(f'        internal static readonly EpsgCoordinateReferenceRecord?[] CoordinateReferences = new EpsgCoordinateReferenceRecord?[{len(catalog["ref_records"])}];')
    lines.append('')
    ...
    def fmt_op(v):
        ...
        return f"(EpsgOperationType){v[0]}, {v[1]}, {v[2]}, {v[3]}, {acc}, \"{esc(v[5])}\", \"{esc(v[6])}\""
    ...
    for idx, ref_record in enumerate(catalog['ref_records']):
        ...
        lines.append(f'                    reference = CoordinateReferences[{idx}] ??= new EpsgCoordinateReferenceRecord({ref_record[0]}, (EpsgCoordinateSystemKind){ref_record[1]}, {ref_record[2]});')
    ...
    lines.append('        {')
    lines.append('            switch (cacheIndex)')
    lines.append('            {')
    for idx, ref_record in enumerate(catalog['ref_records']):
        lines.append(f'                case {idx}:')
        lines.append(f'                    srid = {ref_record[0]};')
        lines.append('                    return true;')
    lines.append('                default:')
    lines.append('                    srid = -1;')
    lines.append('                    return false;')
...
def main():
    ...
    operations = extract_operation_data(zip_path)

    emit(output_path, zip_path.name, catalog, operations)
