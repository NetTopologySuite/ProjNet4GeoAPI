...
def emit(output_path: Path, zip_name: str, catalog, operations):
    ...
    emit_array('CoordinateReferences', 'EpsgCoordinateReferenceRecord', catalog['ref_records'], lambda v: f"{v[0]}, (EpsgCoordinateSystemKind){v[1]}, {v[2]}")
    ...
    for idx, ref_record in enumerate(catalog['ref_records']):
        ...
        lines.append(f'                    reference = CoordinateReferences[{idx}];')
