...
def extract_operation_data(zip_path: Path):
    ...
    return operations, operation_parameters
...
def emit(output_path: Path, zip_name: str, catalog, operations, operation_parameters):
    lines = []
    ...
    def fmt_op(v):
        ...
        return f"(EpsgOperationType){v[0]}, {v[1]}, {v[2]}, {v[3]}, {acc}, \"{esc(v[5])}\", \"{esc(v[6])}\", {v[7]}, {v[8]}"
    ...
    for idx, ref_record in enumerate(catalog['ref_records']):
        ...
        lines.append(f'                    if (CoordinateReferences[{idx}] == null)')
        lines.append('                    {')
        lines.append(f'                        CoordinateReferences[{idx}] = new EpsgCoordinateReferenceRecord({ref_record[0]}, (EpsgCoordinateSystemKind){ref_record[1]}, {ref_record[2]});')
        lines.append('                    }')
        lines.append(f'                    reference = CoordinateReferences[{idx}].Value;')
...
def main():
    ...
    operations, operation_parameters = extract_operation_data(zip_path)

    emit(output_path, zip_path.name, catalog, operations, operation_parameters)
